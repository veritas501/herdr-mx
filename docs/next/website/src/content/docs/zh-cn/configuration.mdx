...
Windows Terminal 支持要求使用包含 OSC 777 桌面通知功能的版本。在该功能进入常规发行版之前，请使用 Windows Terminal Canary。在启动 Herdr 或 WSL2 的 Windows Terminal 配置文件中启用 **允许 OSC 777 (Desktop Notification) 显示 toast 通知**；该设置默认关闭。

当通知来源终端窗格处于焦点时，Windows Terminal 会抑制通知，并应用自身的通知限流。因此测试时应延迟触发通知，并在通知发出前切换到其他窗口。如果 Herdr 本身运行在 tmux 中，请启用 `set -g allow-passthrough on`；只有在包含 Herdr 的 tmux 窗格隐藏时仍需通知，才使用 `all` 代替 `on`。终端投递是尽力而为，Herdr 无法确认 Windows 是否实际显示了 toast。

## 声音

声音提醒由本地前台 Herdr 客户端执行。你可以选择播放音乐、使用外层终端的 Bell，或关闭声音提醒：

```toml
[ui.sound]
enabled = true
mode = "music" # "music" 或 "bell"
path = "sounds/notification.mp3"
done_path = "sounds/done.mp3"
request_path = "sounds/request.mp3"
```

`enabled = false` 会关闭两种模式。`mode = "music"` 保留现有 mp3 行为：`path` 为所有通知设置同一种声音，`done_path` 和 `request_path` 分别覆盖完成和需要输入时的声音。自定义文件必须是 mp3；相对路径从配置文件所在目录解析。

`mode = "bell"` 只写出一个终端 BEL，不启动音频播放器，也不使用 mp3 路径字段。外层终端决定 BEL 是发声、视觉闪烁、任务栏或 Dock 提示、tmux 提醒，还是直接忽略。应用内 Settings 页面提供相同的 Off、Music 和 Bell 三项选择。
...
| `HERDR_DISABLE_SOUND` | 即使 `[ui.sound] enabled = true`，也禁用 Music 和 Bell 声音提醒。 |

...
Windows Terminal での通知には、OSC 777 デスクトップ通知を含むビルドが必要です。この機能が通常リリースに入るまでは Windows Terminal Canary を使用してください。Herdr または WSL2 を起動する Windows Terminal プロファイルで、**Allow OSC 777 (Desktop Notification) to show toast notifications** を有効にします。この設定はデフォルトで無効です。

Windows Terminal は通知元のターミナルペインにフォーカスがある間は通知を抑制し、独自の通知レート制限も適用します。テストでは通知を遅延実行し、発火前に別のウィンドウへ切り替えてください。Herdr 自体を tmux 内で実行する場合は `set -g allow-passthrough on` を有効にします。Herdr を含む tmux ペインが非表示でも通知する必要がある場合に限り、`on` の代わりに `all` を使用してください。ターミナル通知はベストエフォートであり、Windows が実際にトーストを表示したかどうかを Herdr は確認できません。

## サウンド

サウンド通知はローカルのフォアグラウンド Herdr クライアントで実行されます。音楽再生、ホストターミナルの Bell、またはサウンド通知の無効化を選べます:

```toml
[ui.sound]
enabled = true
mode = "music" # "music" または "bell"
path = "sounds/notification.mp3"
done_path = "sounds/done.mp3"
request_path = "sounds/request.mp3"
```

`enabled = false` は両方のモードを無効にします。`mode = "music"` は既存の mp3 動作を維持します。`path` はすべての通知に共通の音を設定し、`done_path` と `request_path` は完了時と入力要求時の音だけを上書きします。カスタムファイルは mp3 でなければならず、相対パスは設定ファイルのディレクトリから解決されます。

`mode = "bell"` はターミナル BEL を 1 回だけ書き込み、オーディオプレーヤーを起動せず、mp3 パスも使用しません。BEL を音、視覚効果、タスクバーや Dock の表示、tmux のアラートとして扱うか、無視するかは外側のターミナルが決定します。アプリ内 Settings でも同じ Off、Music、Bell の選択肢を使用できます。
...
| `HERDR_DISABLE_SOUND` | `[ui.sound] enabled = true` の場合でも Music と Bell のサウンド通知を無効にする。 |

...
Windows Terminal support requires a build that includes OSC 777 desktop notifications. Use Windows Terminal Canary until that support reaches a regular release. In the Windows Terminal profile that launches Herdr or WSL2, enable **Allow OSC 777 (Desktop Notification) to show toast notifications**; the setting is disabled by default.

Windows Terminal suppresses a notification while its originating terminal pane is focused and applies its own notification rate limit, so test by scheduling a notification and switching to another window before it fires. If Herdr itself runs inside tmux, enable `set -g allow-passthrough on`; use `all` instead of `on` only when the tmux pane containing Herdr must notify while hidden. Terminal delivery is best-effort and cannot confirm that Windows displayed the toast.

## Sound

Sound alerts are delivered by the local foreground Herdr client. Choose music playback, the host terminal bell, or disable sound alerts:

```toml
[ui.sound]
enabled = true
mode = "music" # "music" or "bell"
path = "sounds/notification.mp3"
done_path = "sounds/done.mp3"
request_path = "sounds/request.mp3"
```

`enabled = false` disables both modes. `mode = "music"` preserves the existing mp3 behavior: `path` sets one sound for all notifications, while `done_path` and `request_path` override finished and needs-input sounds. Custom files must be mp3 files, and relative paths are resolved from the config file's directory.

`mode = "bell"` writes one terminal BEL and does not start an audio player or use the mp3 path fields. The outer terminal decides whether BEL is audible, visual, a taskbar or Dock indication, a tmux alert, or ignored. The in-app Settings screen exposes the same Off, Music, and Bell choices.
...
| `HERDR_DISABLE_SOUND` | Disable Music and Bell sound alerts even when `[ui.sound] enabled = true`. |
